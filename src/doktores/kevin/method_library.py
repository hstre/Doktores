...
Two engines live here:
  * ``MethodLibrary`` - the seeded store of content-free methods, plus matching by
    affinity (logic) and the transfer of a method onto a target. Improvement 2:
    transfer now *executes* each step against the concrete problem (carrying it out),
    not merely re-labelling it.
  * ``extract_method`` - the loop-closer: when a human marks a run as having worked,
    mine a *new* abstract method out of it and add it to the library. Improvement 5:
    the learned method is tagged with the problem ``domain`` and recorded in a small
    in-memory ledger, so matching can prefer a same-domain learned move.
"""
...
@dataclass
class LearnedMethodLedger:
    """A tiny, in-memory record of methods mined from successful runs.

    Improvement 5: keeps the (method, domain) provenance so the library can prefer a
    learned move from the *same* domain. Deliberately in-memory and append-only - no
    file I/O is required for tests; a real deployment may serialise it to JSONL, but
    the engines never depend on that.
    """

    entries: list[tuple[str, str]] = field(default_factory=list)  # (method_id, domain)

    def record(self, method: Method) -> None:
        self.entries.append((method.id, method.learned_domain))

    def as_jsonl(self) -> str:
        import json

        return "\n".join(
            json.dumps({"method_id": mid, "domain": dom}) for mid, dom in self.entries
        )
...
class MethodLibrary:
    ...
    def __init__(self, methods: tuple[Method, ...] = SEED_METHODS) -> None:
        ...
        self.ledger = LearnedMethodLedger()
    ...
    def match(
        ...
        *,
        ...
        Improvement 5: when ``prefer_domain`` is given, a learned method whose
        ``learned_domain`` matches gets a deterministic tie-breaking boost, so a move
        that already worked in this domain is preferred over an equally-shaped one
        that did not - without ever letting domain *content* cross the boundary.
        """
        ...
        for m in self._methods.values():
            ...
            if overlap:
                domain_bonus = 1 if (prefer_domain and m.learned_domain == prefer_domain) else 0
                scored.append((overlap, domain_bonus, m))
        scored.sort(key=lambda t: (-t[0], -t[1], t[2].id))
        return [m for _, _, m in scored[:top_k]]

    # -- transfer (structure = logic, wording/execution = LLM) -------------- #
    def transfer(
        self,
        llm: LLMClient,
        method: Method,
        ...
        *,
        ...
        """Re-read AND carry out a method's content-free steps against a target.

        The *mapping* (one mapped step per abstract step, in order) is deterministic
        structure. Improvement 2: instead of only re-labelling each step, the LLM now
        *executes* it against the concrete problem and its named ``variables`` - so a
        "push variables to extremes" step actually reports what happens at the
        extremes of a named variable. The original domain content of the method never
        appears - only its shape crosses the boundary.
        """
        mapped = tuple(
            llm.execute_step(problem_statement or target_text, target_text, step, variables)
            for step in method.steps
        )
        rationale = (
            f"Method '{method.name}' (from {method.origin}) transferred by shape, "
            f"not content: its steps executed against the target."
        )
...
def extract_method(
    ...
    Improvement 5: the learned method carries the run's ``problem.domain`` as
    ``learned_domain`` (credit/provenance only - never transferable content), so the
    library can later prefer same-domain learned moves. A ``name`` may be supplied by
    the human; if not, one is derived from the (axis, move) shape - which also makes
    extraction idempotent, since the same shape yields the same method id.
    """
    ...
    return Method(
        ...
        learned_domain=run.problem.domain,
    )
