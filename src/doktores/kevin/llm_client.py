...
logic**:
  ...
  * The LLM proposes spaces, writes wild variants, phrases/executes method
    transfers, critiques candidates and *reads* candidates into structured signals.
...
the same Protocol without touching any engine. The real-client toggle is
``DOKTORES_USE_REAL_LLM=1`` with key priority
...
class LLMClient(Protocol):
    ...
    def write_variant(
        self, problem: Problem, space: SolutionSpace, move: WildMove, *, persona: str = ""
    ) -> str:
        """Write one wild variant. May be absurd - that is the job.

        Grounding: the problem's ``evidence``/``anchors`` are woven in so the
        variant references the concrete field. ``persona`` lets a multi-persona
        ensemble vary the voice deterministically.
        """
    ...
    def execute_step(
        self, problem_statement: str, target_text: str, step: str, variables: tuple[str, ...]
    ) -> str:
        """*Carry out* an abstract method step against the concrete problem.

        Unlike ``phrase_transfer`` (which only re-labels the step), this actually
        works the step through against the named ``variables`` - e.g. for a
        "push variables to extremes" step it says what happens at the extremes.
        """

    def critique(self, problem: Problem, candidate_text: str) -> dict:
        """Attack a candidate and return a structured weakness. Language only."""
...
class MockLLM:
    ...
    # Distinct persona voices - each shifts the hash seed and the framing so the
    # ensemble harvests genuine divergence while staying replay-stable.
    _PERSONAS = {
        "": "",
        "skeptic": "Pressed by a skeptic: ",
        "visionary": "Pushed to the visionary extreme: ",
        "engineer": "Re-grounded by an engineer: ",
        "contrarian": "Flipped by a contrarian: ",
    }
    ...
    def write_variant(
        ...
        s = _seed(problem.statement, space.id, move.value, persona)
        x = self._DISTANT[s % len(self._DISTANT)]
        template = self._WILD_TEMPLATES[move]
        text = template.format(p=problem.statement, x=x)
        prefix = self._PERSONAS.get(persona, f"As {persona}: " if persona else "")
        # Grounding: deterministically pick a concrete anchor/evidence item and weave
        # it in so downstream connectivity signals actually improve.
        ground = (*problem.anchors, *problem.evidence)
        if ground:
            chosen = ground[s % len(ground)]
            text = f"{text} Grounded in {chosen}."
    ...
    def execute_step(
        ...
        # *Carry out* the step against the concrete problem. Deterministic and
        # concrete-looking; mentions a variable when variables are present.
        s = _seed(problem_statement, target_text, step)
        if variables:
            var = variables[s % len(variables)]
            low = step.lower()
            if "extreme" in low or "zero" in low or "limit" in low:
                return (
                    f"Executed: at the extremes of '{var}' (both maximal and at zero), "
                    f"the behaviour of '{problem_statement}' flips - the rule reads off the gap."
                )
            return (
                f"Executed against '{var}': {step.rstrip('.')} - concretely, vary '{var}' "
                f"and observe the effect on '{problem_statement}'."
            )
        return f"Executed: {step.rstrip('.')} - worked through against '{problem_statement}'."

    def critique(self, problem: Problem, candidate_text: str) -> dict:
        s = _seed(problem.statement, candidate_text)
        weaknesses = (
            "no falsifiable prediction is stated",
            "the mechanism is asserted, not traced",
            "it does not say what would count as evidence against it",
            "it leans on the analogy without checking the disanalogies",
        )
        weakness = weaknesses[s % len(weaknesses)]
        fix = (
            "state one concrete observation that, if seen, would refute it",
            "name the variable whose change drives the claimed effect",
            "specify the cheapest measurement that discriminates it from the obvious answer",
            "list the points where the borrowed structure breaks down",
        )[s % 4]
        return {"weakness": weakness, "suggested_fix": fix}
    ...
    def read_signals(self, problem: Problem, candidate_text: str) -> dict:
        ...
        return {
            ...
            "has_falsifiable_claim": ("if " in text or "bet that" in text
                                      or "refute" in text or s % 2 == 0),
            "internal_contradiction": (s % 7 == 0),
            "has_concrete_mechanism": ("mechanism" in text or "lever" in text or "->" in text
                                       or "applied to" in text or "executed" in text),
            "anchors": anchors,
            "overlaps_known": any(
...
class OpenAICompatibleLLM:
    ...
    Matches the rest of the ecosystem: ``DEEPSEEK_API_KEY`` > ``OPENROUTER_API_KEY``
    > ``OPENAI_API_KEY``; the client selects ``deepseek-chat`` or ``gpt-4o``
    accordingly. This class is the *only* place a network call happens - the engines
    never change.
    ...
    def __init__(self, model: str | None = None, base_url: str | None = None,
        ...
        if deepseek:
            ...
            raise RuntimeError(
                "No LLM key found. Set DEEPSEEK_API_KEY, OPENROUTER_API_KEY or "
                "OPENAI_API_KEY, or unset DOKTORES_USE_REAL_LLM to use the MockLLM."
            )
    ...
    def write_variant(
        ...
        voice = f" Adopt the voice/stance of a {persona}." if persona else ""
        system = (
        ...
        ground = ", ".join((*problem.anchors, *problem.evidence)) or "none given"
        user = (
            f"Problem: {problem.statement}\n"
            f"Region to explore: {space.label} - {space.description}\n"
            f"Concrete anchors/evidence to reference at least one of: {ground}\n"
            f"Creative move to use: {move.value.replace('_', ' ')}.\n"
            "Produce one wild variant using exactly that move, weaving in one anchor."
        )
    ...
    def execute_step(
        ...
        system = (
            "You CARRY OUT an abstract thinking step against a concrete problem. "
            "Do not merely re-label it - actually work it through and report the result "
            "in one or two concrete sentences, referencing the named variables if any."
        )
        vars_ = ", ".join(variables) or "none named"
        user = (
            f"Problem: {problem_statement}\nIdea: {target_text}\n"
            f"Variables/levers: {vars_}\nStep to execute: {step}"
        )
        return self._chat(system, user, temperature=0.4).strip()

    def critique(self, problem: Problem, candidate_text: str) -> dict:
        system = (
            "You ATTACK an idea to find its single biggest weakness. You do not score. "
            'Reply as JSON: {"weakness": short string, "suggested_fix": short string}.'
        )
        user = f"Problem: {problem.statement}\nIdea: {candidate_text}"
        data = self._parse_json(self._chat(system, user, temperature=0.3, json=True))
        return {
