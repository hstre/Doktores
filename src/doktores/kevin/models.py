"""Core data model for Kevin (vendored into Doktores).
...
Following the ecosystem convention (Doktores / Joni / AleXiona): **LLM for
language, rules for logic**. Everything in this module is plain, deterministic
data. All scoring, routing and selection live in the engines and operate on
...
class Verdict(StrEnum):
    """The selector's closed verdict set. Mirrors the gate discipline."""
...
class Problem:
    ...
    *what is under-explored* instead of re-suggesting the obvious.
    ...
    Grounding (improvement 1): ``evidence`` are concrete facts/observations and
    ``anchors`` are concrete named entities of the problem field. They are woven
    into the wild brother's variants so candidates connect back to the field
    instead of floating free. ``variables`` (improvement 2) are the concrete
    quantities/levers a method can be *executed* against.
    """
...
class Variant:
    ...
    ``persona`` (improvement 8) records which deterministic persona produced the
    variant, so a multi-persona ensemble can harvest divergence.
    """
...
class Method:
    ...
    ``learned_domain`` (improvement 5) tags methods mined from a successful run
    with the problem domain they were learned in, so matching can optionally prefer
    a same-domain learned method. Seed methods leave it empty.
    """
...
class Candidate:
    ...
    ``hardened_from`` (improvement 4a) is set on a candidate produced by the
    falsification loop - a revised version that addresses a weakness found in the
    candidate whose id is recorded here.
    """
    ...
    method_name: str = ""
    hardened_from: str | None = None

    @property
    def id(self) -> str:
        return stable_id(
            "cand", self.variant_id, self.transfer_id or "raw", self.hardened_from or ""
        )
...
class Evaluation:
    ...
    verdict: Verdict
    reason: str
    pareto: bool = False     # improvement 4b: on the (novelty, testability) Pareto front
...
class CreativeRun:
    ...
    problem: Problem
    space_prediction: dict | None = None   # where the explorer predicts the open regions are
    ...
    hardened: list[str] = field(default_factory=list)   # ids of hardening-loop candidates
    pareto_ids: list[str] = field(default_factory=list)  # candidate ids on the Pareto front
